# Copyright (c) Facebook, Inc. and its affiliates.share_decoder_input_output_embed
#
...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # parser.add_argument('--hidden-path', type=str, default=None)
        # parser.add_argument('--test-tokens', type=int, default=5)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        self.args = args
        # self.token_idx = 0
        # self.hidden_states = np.memmap(args.hidden_path, dtype=np.float32, mode='w+', shape=(args.test_tokens, 1024))
        super().__init__(dictionary)
        self.register_buffer("version", torch.Tensor([3]))
    ...
    def forward(
        ...
        if self.use_knn_datastore:
            ...
            if self.label_count_as_feature:
                return x, extra, knn_prob, knn_lambda, knn_dists, knn_index, last_hidden, label_counts
            else:
...
@register_model_architecture("transformer", "transformer_wmt19_de_en_postnorm")
def transformer_wmt19_de_en_postnorm(args):
    #args.encoder_normalize_before = getattr(args, "encoder_normalize_before", False)
    #args.decoder_normalize_before = getattr(args, "decoder_normalize_before", False)
    transformer_wmt19_de_en(args)

@register_model_architecture("transformer", "transformer_wmt19_de_en_pronorm")
def transformer_wmt19_de_en_pronorm(args):
    #args.encoder_normalize_before = getattr(args, "encoder_normalize_before", False)
    #args.decoder_normalize_before = getattr(args, "decoder_normalize_before", False)
    transformer_wmt19_de_en(args)


@register_model_architecture("transformer", "transformer_wmt19_de_en_prenorm")
def transformer_wmt19_de_en_prenorm(args):
    args.encoder_normalize_before = getattr(args, "encoder_normalize_before", True)
    args.decoder_normalize_before = getattr(args, "decoder_normalize_before", True)
    transformer_wmt19_de_en(args)
