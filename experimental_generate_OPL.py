...
def _main(args, output_file):
    target_file = open(args.target_path, 'w')
    index_file = open(args.index_path, 'w')
    # logging.basicConfig(
    ...
    has_target = True
    wps_meter = TimeMeter()
    hidden_states = np.memmap(args.hidden_path, dtype=np.float32, mode='w+',
                                shape=(args.test_tokens, 1024))
    token_idx = 0
        ...
        for i, sample_id in enumerate(sample["id"].tolist()):
            ...
            for j, hypo in enumerate(hypos[i][: args.nbest]):
                ...
                if not args.quiet:
                    ...
                    if args.report_targets:
                        print("{}".format(" ".join([str(index) for index in sample['target'][0].tolist()])), file=target_file)
                    if args.knn_record_index:
                        knn_indexes = hypo["knn_index"].transpose(0, 1).tolist()
                        for knn_index in knn_indexes:
                            print(" ".join([str(index) for index in knn_index]), file=index_file)
                    
                    lprobs = hypo["lprobs"]
                    hiddens = hypo["hidden_state"]
                    
                    hidden_states[token_idx:token_idx+hiddens.shape[0]] = hiddens.detach().cpu().numpy().astype(np.float16)

                    token_idx += lprobs.shape[0]
